...
11. **`openclaw plugins update` complains about missing installation** → OpenClaw only updates plugins that are recorded in `plugins.installs` (see [CLI plugins](https://docs.openclaw.ai/cli/plugins)). We bundle trading-recipe by copying it into the image; bootstrap copies it into state only when missing (copyDirIfMissing) so that when the user explicitly runs `openclaw plugins update`, the update persists across restarts. Bootstrap also writes `plugins.installs["trading-recipe"]` so the update command works.
