...
11. **trading-recipe plugin** → Installed at runtime by bootstrap via `openclaw plugins install @senpi/trading-recipe --pin` when `openclaw.json` exists and the plugin is enabled (not in the Docker image). So `plugins.installs` and updates work without a synthetic config. Set `SENPI_TRADING_RUNTIME_ENABLED=false` to disable.
